        ...
        if factor == 'turn_over_rate':
            ...
            dictionary = {
                s: history_bars(
                    ...
                    bar_count=n,
                    fields=factor,
                    frequency="1d").mean() for s in stocklist}
...
def determine_filter_technical_indicator_for_one_stock(
        context, stk, bar_count):
    """
    检查指定股票是否通过筛选。
    context: 上下文
    stk: 股票代码
    ...
    EMA__timeperiod_10_ = pd.DataFrame(
        np.asarray(EMA(**({'timeperiod': 10}))).T, columns=EMA.info
        ['output_names'])

    EMA.input_arrays = inputs

    EMA__timeperiod_24_ = pd.DataFrame(
        np.asarray(EMA(**({'timeperiod': 24}))).T, columns=EMA.info
        ['output_names'])
...
def sort_indicators(stocklist, context, bar_count=100):
    ...
    final_scores = pd.Series(
        np.asarray(
            np.matmul(
                indicator_score_dataframe.values,
                weights)),
        index=stocklist)
...
def rebalance_plan(context, bar_dict):
    ...
    bar_dict: 当日行情数据
    """
    stock_pool = get_stock_pool(context, bar_dict)

    stock_pool = sort_indicators(stocklist=stock_pool, context=context)
    ...
    context.sell_all = current_holdings - stock_pool
    context.buy_all = stock_pool - current_holdings
...
def rebalance_plan_sell(context, bar_dict):
    ...
    bar_dict: 当日行情数据
    """
    # for stk in adjust_position:
    #     print('%s:Adjusting %s'%(context.now.date(),stk))
    #     order_target_percent(stk, target_percent, MarketOrder())

...
def handle_bar(context, bar_dict):
    ...
    if hsIncrease < 0 and zzIncrease < 0:#hs下跌，zz也下跌
        if hsQuality > 0:#has stock hs
            order_target_percent(context.stocks[0],0)
            logger.info("卖出沪深300")
        if zzQuality > 0:
            order_target_percent(context.stocks[1],0)
            logger.info("卖出中证500")
        if gzQuality <= 0.001:#buy 
            order_target_percent(context.stocks[2],0.99)
            logger.info("买入国债")
    
    elif hsIncrease < zzIncrease and zzIncrease>0:
        if hsQuality > 0:#hs没有zz好，zz肯定为正
            order_target_percent(context.stocks[0],0)
            logger.info("卖出沪深300")
        if gzQuality > 0:
        ...
        if zzQuality <= 0.001:
            ...
            logger.info("买入中证500")
    elif hsIncrease > zzIncrease and hsIncrease>0:
        if zzQuality > 0:#hs没有zz好，zz肯定为正
            order_target_percent(context.stocks[1],0)
            logger.info("卖出中证500")
        if gzQuality > 0:
            order_target_percent(context.stocks[2],0)
